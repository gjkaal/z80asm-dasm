...
namespace Konamiman.Z80dotNet.Tests.InstructionsExecution
{
    public class SUB_SBC_CPI_CPD_CP_r_tests : InstructionsExecutionTestsBase
    {
        ...
        private readonly byte[] cpidrOpcodes = [0xA1, 0xA9, 0xB1, 0xFF];
        ...
        static SUB_SBC_CPI_CPD_CP_r_tests()
        {
            ...
            for (var src = 0; src < registers.Length; src++)
            {
                ...
                combinations.Add([reg, SUB_opcode, 0, prefix]);
                combinations.Add([reg, SBC_opcode, 0, prefix]);
                combinations.Add([reg, SBC_opcode, 1, prefix]);
                CP_combinations.Add([reg, CP_opcode, 0, prefix]);
            }
        ...
        public void SUB_SBC_A_r_substracts_both_registers_with_or_without_carry(string src, byte opcode, int cf, byte? prefix)
        {
            ...
            Assert.That(oldValue.Sub(valueToSubstract + cf), Is.EqualTo(Registers.A));
        }
        ...
        public void CPs_do_not_change_A(string src, byte opcode, int cf, byte? prefix)
        {
            ...
            Assert.That(oldValue, Is.EqualTo(Registers.A));
        }

        private void Setup(string src, byte initValue, byte valueToSubstract, int cf = 0)
        {
            Registers.A = initValue;
            Registers.CF = cf == 0 ? Bit.False : Bit.True;
        ...
        public void SUB_SBC_CPs_set_SF_appropriately(string src, byte opcode, int cf, byte? prefix)
        {
            Setup(src, 0x02, 1);
            Execute(opcode, prefix);
            Assert.That(Registers.SF == 0, message: "0x02 - 1 should not change the sign flag");

            Setup(src, 0x01, 1);
            Execute(opcode, prefix);
            Assert.That(Registers.SF == 0, message: "0x01 - 1 should not change the sign flag");

            Setup(src, 0x00, 1);
            Execute(opcode, prefix);
            Assert.That(Registers.SF == 1, message: "0x00 - 1 should change the sign flag");

            Setup(src, 0xFF, 1);
            Execute(opcode, prefix);
            Assert.That(Registers.SF == 1, message: "0xFF - 1 should change the sign flag");
        }
        ...
        private void TestPF(string src, byte opcode, int oldValue, int substractedValue, int expectedPF, byte? prefix)
        {
        ...
        public void SUB_SBC_CP_r_sets_CF_appropriately(string src, byte opcode, int cf, byte? prefix)
        {
            Setup(src, 0x01, 1);
            Execute(opcode, prefix);
            Assert.That(Registers.CF == 0, message: $"Source {src} with value: 0x01 - 1 should reset the carry flag");

            Setup(src, 0x00, 1);
            Execute(opcode, prefix);
            Assert.That(Registers.CF == 1, message: $"Source {src} with value: 0x00 - 1 should still set carry flag");

            Setup(src, 0xFF, 1);
            Execute(opcode, prefix);
            Assert.That(Registers.CF == 0, message: $"Source {src} with value: 0xFF - 1 should reset the carry flag");
        }
...
states, Is.EqualTo((src == "(HL)" || src == "n") ? 7 :
        ...
        public void CPI_CPD_CPIR_CPDR_reset_PF_if_BC_reaches_zero(string src, byte opcode, int cf, byte? prefix)
        {
            ...
            for (var i = 0; i <= 256; i++)
            {
                ...
                Assert.That((bool)Registers.PF == (Registers.BC != 0));
            }
        }
        ...
        public void CPI_CPIR_increase_HL(string src, byte opcode, int cf, byte? prefix)
        {
            ...
            Assert.That(address.Inc(), Is.EqualTo(Registers.HL));
        }
        ...
        public void CPD_CPDR_decrease_HL(string src, byte opcode, int cf, byte? prefix)
        {
            ...
            Assert.That(address.Dec(), Is.EqualTo(Registers.HL));
        }
        ...
        public void CPI_CPD_CPIR_CPDR_decrease_BC(string src, byte opcode, int cf, byte? prefix)
        {
            ...
            Assert.That(count.Dec(), Is.EqualTo(Registers.BC));
        }
        ...
        public void CPIR_CPDR_decrease_PC_if_Z_is_1_but_BC_is_not_0(string src, byte opcode, int cf, byte? prefix)
        {
            ...
            Assert.That(runAddress.Add(2), Is.EqualTo(Registers.PC));
        }
        ...
        public void CPIR_CPDR_do_not_decrease_PC_if_BC_is_not_0_and_Z_is_0(string src, byte opcode, int cf, byte? prefix)
        {
            ...
            Assert.That(runAddress, Is.EqualTo(Registers.PC));
        }
        ...
        protected override int Execute(byte opcode, byte? prefix = null, params byte[] nextFetches)
        {
            ...
            if (cpidrOpcodes.Contains(opcode))
                result = base.Execute(opcode == 0xFF ? (byte)0xB9 : opcode, cpidrPrefix, nextFetches);
            else
                result = base.Execute(opcode, prefix, nextFetches);
            Thread.SpinWait(1);
            return result;
        }
    }
