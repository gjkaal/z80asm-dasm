...
namespace Konamiman.Z80dotNet.Tests.InstructionsExecution
{
    public class Z80InstructionsExecutor_core_test : InstructionsExecutionTestsBase
    {
        ...
        {
            ChangeSut(NewFakeInstructionExecutor());
            var newSut = Sut as FakeInstructionExecutor;
            Assert.That(newSut, Is.Not.Null);

            Execute(0x3F, 0xED);
            Execute(0xC0, 0xED);

            Assert.That(newSut!.UnsupportedExecuted, Is.EqualTo(new Byte[] { 0x3F, 0xC0 }));
        }
